...
@onready var max_particles_per_tick: int = ProjectSettings.get_setting(MAX_PARTICLES_PER_TICK_SETTING_PATH, 5)
...
func _ready() -> void:
	for bus: int in AudioServer.bus_count:
		...
		var volume: float = ProjectSettings.get_setting(get_volume_setting_path(bus_name), 0.0)
